...
class ArtDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (info.equals("new")) {
            ...
            val selectedImageBackground = BitmapFactory.decodeResource(applicationContext.resources,R.drawable.camera)
    ...
    private fun registerLauncher() {
                ...
                if (intentFromResult != null) {
                    ...
                    try {
                        if (Build.VERSION.SDK_INT >= 28) {
                            ...
                            selectedBitmap = MediaStore.Images.Media.getBitmap(this@ArtDetailActivity.contentResolver, imageData)
                            binding.imageView.setImageBitmap(selectedBitmap)
                        }
                    } catch (e: IOException) {
                        e.printStackTrace()
        ...
        permissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { result ->
            if (result) {
                ...
                //permission denied
                Toast.makeText(this@ArtDetailActivity, "Permisson needed!", Toast.LENGTH_LONG).show()
            }
        }
    }
